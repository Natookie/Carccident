...
public class CarLogic : MonoBehaviour
{
    ...
    [SerializeField] private float reactionTimeVariation = 0.3f;
    [Space(5)]
    [SerializeField] private float accelerationRate = 5f;
    [SerializeField] private float accelerationVariation = 1.5f;
    [Space(5)]
    ...
    [Space(5)]
    [SerializeField] private float exitOffset = 5f;
    [SerializeField] private float bezierOffset = 2f;

    [Header("RAYCAST SETTINGS")]
    [SerializeField] private float raycastDistance = 30f;
    [SerializeField] private LayerMask obstacleLayerMask;
    [SerializeField] private float minFollowDistance = 4f;
    [SerializeField] private float followDistanceVariation = 1.5f;
    ...
    void Update(){
        ...
        if(isAligningAfterTurn) AlignToLaneRotation();
    ...
    Vector3 GetTurnExitPoint(){
        ...
        float exitDist = exitOffset;

        int baseID = laneID & ~1;

        switch(baseID){
            case 0:return new Vector3(tx - exitDist, yPos, tz);
            case 2:return new Vector3(tx + exitDist, yPos, tz);
            case 4:return new Vector3(tx, yPos, tz + exitDist);
            case 6:return new Vector3(tx, yPos, tz - exitDist);
    ...
    Vector3 GetTurnApexPoint(){
        float yPos = transform.position.y;
        Vector3 entryDir = originalMoveDirection.normalized;
        Vector3 exitDir = (GetTurnExitPoint() - turnTarget.position).normalized;
        Vector3 bisector = (entryDir + exitDir).normalized;
        return turnTarget.position - bisector * bezierOffset + Vector3.up * yPos;
    }
